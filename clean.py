...
class FingerprintImageEnhancer:
    ...
    def __ridge_filter(self):
        ...
        if unfreq.size == 0:                       # <‑‑ add this check
            # fallback: treat the normalised image as the result,
            # or simply return without modifying self._binim
            self._binim = self._normim > 0         # or any other default
            return
...
def enhance_fingerprint(
    ...
    if enhanced_output.dtype == bool:
        binary_img = (enhanced_output.astype(np.uint8) * 255)
    else:
        binary_img = enhanced_output.astype(np.float32)
    return binary_img