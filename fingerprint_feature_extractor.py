...
class FingerprintFeatureExtractor(object):
    ...
    def __make_disp(self):
        if len(self._orig.shape) == 2:            # grayscale
            return cv2.cvtColor(self._orig, cv2.COLOR_GRAY2BGR)
        else:                                     # already colour
    ...
    def saveResult(self, FeaturesTerm, FeaturesBif):
        DispImg = self.__make_disp()
        for feat in FeaturesTerm:
            r, c = feat.locX, feat.locY        # (row, col) from extractor 
            cv2.circle(DispImg, (c, r), 3, (0, 0, 255), 3)

        for feat in FeaturesBif:
            r, c = feat.locX, feat.locY
            cv2.circle(DispImg, (c, r), 3, (255, 0, 0), 3)

        return cv2.cvtColor(DispImg, cv2.COLOR_BGR2RGB)

def extract_minutiae_features(img, spuriousMinutiaeThresh=10, invertImage=False):
    ...
    feature_img=feature_extractor.saveResult(FeaturesTerm, FeaturesBif)
    
    return FeaturesTerm, FeaturesBif, feature_img